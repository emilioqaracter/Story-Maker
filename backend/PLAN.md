...
| §7.1 a §7.3 Flujos, reparación y cuarentena | T14, T15; el flujo completo en T24; el modo permisivo de `breve`, `corta` y `prueba` en T53 | Cubierto |
...
| §9.3 Puertas | T12, T14, T15; la mitad de juicio en T20 y T22; prohibidas y cronología antes de congelar en T41 y T46; elementos en el cierre en T47; el modo permisivo en T53 | Cubierto |
| §10 Escritura de canon | T10, T11; el retcon en T23; comprobaciones antes de toda congelación en T41 y T46 | Cubierto |
| §11 Observabilidad | T9; las trece señales en T22; Langfuse en T40 y T48; audit log en T45; coste y tiempo de redacción en RI-03 en T53 | Cubierto |
...
| T53 | — | RF-274, RF-284, RF-285, RD-50 | VER-05, VER-08 | `commons/types/length.py`, los perfiles y `lenient` (D-136), con `corta` (D-139); `canon/brief.py`, `length_profile` y el rango de obra por perfil; `planning/outline/types.py`, que reexporta los perfiles en vez de `CHAPTER_WORDS`, `check.py` y `prompts.py`; `orchestration/compose.py`, el número de capítulos, `engine.py`, el rango de escena del Escritor y de `check.format`, y `loop.py`, `check_chapter_length` en `_approve_chapter`; `verification/checks/deterministic.py`; `verification/jury/verdict.py` y `prompts.py`, el umbral y el mínimo de cita del perfil, y `generation/sports/narrate.py`, el resultado obligatorio del Especialista (D-128); `evals/brief_table.py`, el umbral registrado en `jury`; `backend/evals/briefs/01` a `05` con `prueba`, y sus pruebas; con `lenient` (RF-284), `orchestration/engine.py`, el juez sin reintento por cita, `loop.py`, la escena aceptada, la puerta de capítulo, el Árbitro, el delta vacío, la reparación agotada, la replanificación con el perfil del brief también en la cuarentena y el cierre, y `verification/jury/verdict.py`, el Jurado sin segunda ronda, con `orchestration/test_lenient.py`; en todo perfil (D-140), `planning/scene_spec/prompts.py`, el elenco filtrado al canon con `known` desde `engine.specs_for`, y `loop.py`, `_freeze` sin presentes que no existen y lo aceptado de nuevo por la puerta 1 con `lenient`, con `planning/scene_spec/test_prompts.py` y `orchestration/test_lenient.py`; con RF-285, `orchestration/routes.py`, `cost_usd` y `writing_ms` en `RunState`, con `orchestration/test_routes.py`, y `frontend/run-health/Status.tsx`, `formatDuration` y `formatCost`, con `Status.test.tsx` (D-138); `backend/openapi.json` y el cliente regenerado en `frontend/commons/api/` | 3 · S-MINI · después de S-FIX y S-S |
...
### T53 · Perfiles de extensión `prueba`, `breve` y `corta`

| Requisitos | RF-274, RF-284, RF-285, RD-50 |
|---|---|
| **Puerta** | Una tirada con dobles de cada brief de evaluación cierra con 3 capítulos de 1 escena y entre 1.200 y 1.800 palabras; un brief `breve` planifica 10 capítulos y uno `corta` 5, y el ejemplo de escaleta de cada uno pasa `outline.check` (`orchestration/test_profiles.py` y `planning/outline/test_check.py`, D-132, D-139); la entrevista elige `breve`, `corta` o `novela` por la extensión (`brief/test_interview.py`, D-137); un brief `novela` sin perfil se comporta igual que hoy; una tirada `breve` con dobles cuyo Continuista, Jurado y Árbitro siempre fallan congela los 10 capítulos y cierra, y con `novela` los mismos dobles abortan como hoy (`orchestration/test_lenient.py`, D-136); un elenco con una entidad inventada no tumba la congelación y el elenco se filtra a las entidades del canon conservando el POV (`orchestration/test_lenient.py::test_un_elenco_con_una_entidad_inventada_no_tumba_la_congelacion` y `planning/scene_spec/test_prompts.py::test_el_elenco_se_filtra_a_las_entidades_del_canon_y_conserva_el_pov`, D-140); RI-03 da el coste de los `call` de la traza y el tiempo de redacción por invocación, y la vista de estado los muestra (`orchestration/test_routes.py` y `frontend/run-health/Status.test.tsx`, D-138) |
