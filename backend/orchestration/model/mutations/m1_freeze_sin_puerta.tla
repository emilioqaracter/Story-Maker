------------------------------ MODULE m1_freeze_sin_puerta ------------------------------
\* MUTACION m1_freeze_sin_puerta: un delta que el Arbitro rechaza se congela igual
...
\* MUTACION: el rechazo no vuelve a la reparacion, va a congelar.
MutDeltaRejected ==
    /\ state = "Arbitrating"
    /\ deltaClean' = FALSE
    /\ state' = "Freezing"
    /\ UNCHANGED << scene, sceneAttempts, chapterAttempts, arcReplans, canonWritten,
                    gatesPassed, juryPassed, openS1, repaired, inFlight, jurorsIn,
                    jurorsDone, retcons, lastClosed >>

\* MUTACION: congelar sin mirar las puertas.
MutFreeze ==
    /\ state = "Freezing"
    /\ canonWritten' = TRUE
    /\ state' = "Frozen"
...
ChapterStep ==
    ...
    /\ \/ Ch!WriteScene \/ Ch!SceneFailsRetry \/ Ch!SceneFailsRespec
       ...
       \/ Ch!Extract \/ Ch!DeltaAccepted \/ MutDeltaRejected   \* MUTACION
...
ScenePasses ==
    /\ phase = "Running"
    /\ Ch!ScenePasses
    /\ drafts' = [ i \in 1..Scenes |-> IF i = scene THEN "ok"
