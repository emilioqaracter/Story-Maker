...
|---|---|---|---|---|
| **9.0** | 2026-09-16 | _sin commitear_ | **Las novelas pasan a ser de un solo protagonista y con estructura de tres actos.** Fuera `relacion.yaml` y las cinco etapas de pareja; entra `arco.yaml` con protagonista, **meta**, **obstáculo**, **precio** y los tres actos, que se reparten sobre la época (25/50/25) y **se calculan por fecha**, no se declaran. V16 comprueba el arco, V17 que los tres actos tengan escenas y V18 que la última caiga en el desenlace. La quinta dimensión de la rúbrica pasa de `quimica` a `avance`, y `dialogo` a `voz`. | El género romántico obligaba a dos protagonistas y a un arco de pareja que no encaja con lo que se quiere contar: una recuperación, una consagración, un retiro. Los tres actos sustituyen a las cinco etapas porque hacen el mismo trabajo —decirle al escritor en qué momento de la historia está— sin presuponer de qué va. Y siguen siendo **derivados**: la regla de que lo calculable no se guarda vale igual para un arco que para una edad. |
| **9.0** | 2026-09-16 | _sin commitear_ | **G2 deja de sumar: decide el crítico.** `critic-quality` devuelve `{pasa, motivo}` y el script releva ese veredicto. Lo que el script sigue comprobando: que haya veredicto, que traiga motivo, que las notas citen y que la crítica no sea más vieja que la prosa. La rúbrica se mantiene como **medida**, no como puerta. | Decisión del usuario, con el precio dicho por delante: un modelo que se autoevalúa aprueba casi todo, y sin umbral no hay un listón comparable entre corridas. Lo que se conserva es lo que un modelo no puede juzgar sobre sí mismo — si su crítica describe el texto que hay ahora, y si su veredicto es accionable. Un `no pasa` sin motivo gasta un intento entero, porque el corrector no tiene a qué agarrarse. |
| **9.0** | 2026-09-16 | _sin commitear_ | **La época pasa a ser cualitativa** y se van cinco reglas duras: la edad a la fecha (V3), el cumpleaños (V4), lo que cada personaje sabe (V7), las figuras reales (V9) y el hito del calendario (V20). `calendario.yaml` y `real-figures.yaml` desaparecen; las fuentes dejan de ser obligatorias. | El harness pedía precisión de almanaque —qué día se jugó cada partido— y eso costaba la mitad del tiempo de investigación sin cambiar una línea de prosa. Lo que se le pide ahora a la época es que sea **coherente**: en 1800 no hay móviles, en 2010 sí hay internet. Los datos no desaparecen del canon, dejan de ser puerta: le siguen llegando al escritor en el contexto resuelto, y las contradicciones que queden las caza el crítico de continuidad, que para eso lee. |
| **9.0** | 2026-09-16 | _sin commitear_ | **Se borra el conductor en Python** (`legado/`) y la UI queda con un solo botón. Nuevo `harness/scripts/reportar.py`: manda a Langfuse una traza por escena con sus intentos, sus puertas y la rúbrica como scores. | Un solo camino es más fácil de explicar y de mantener, y el valor de la comparación ya está documentado. Lo que sí se rescató del conductor viejo es su código de Langfuse: el hook de Claude Code cuenta la conversación (cómo trabajó el agente) y `reportar.py` cuenta la novela (cómo salió el libro). Son dos preguntas distintas y hacían falta las dos. |
| **8.1** | 2026-09-16 | _sin commitear_ | **La entrevista de doce preguntas desaparece.** Sus derivaciones pasan a `harness/derivaciones.py` (librería, un solo sitio donde se deriva) y el arranque lo lleva la skill `preparar-libro`: el orquestador exprime la idea, deriva lo derivable, manda al `researcher` a por lo averiguable y pregunta **una sola tanda** solo lo que cambia el libro. | Doce preguntas fijas preguntaban lo mismo para boxeo en 1950 que para natación en 2024, y varias eran del cubo equivocado: la edad se calcula y los anacronismos se investigan — preguntarlos es pedirle al usuario que haga de calculadora o de enciclopedia. El criterio nuevo es explícito: **solo se pregunta lo que solo una persona puede decidir**, y solo la parte que no se deduce de lo que ya dijo. |
| **8.1** | 2026-09-16 | _sin commitear_ | **La traza la escriben las propias puertas**, en `common.emitir()`, no el conductor. | Con dos conductores (Claude Code y el bucle en Python), una traza escrita por el conductor significaba que la mitad del sistema no dejaba rastro según quién lo corriera — y una métrica que depende de quién mira no es una métrica. Anotar en `emitir()` además hace que una puerta nueva no pueda olvidarse de registrarse: si emite, queda. La división queda limpia: **el JSONL cuenta lo que hizo el código, Langfuse lo que hizo el modelo**. |
| **8.1** | 2026-09-16 | _sin commitear_ | El conductor en Python se aparta a `legado/` (con `demo.py` y `observabilidad.py`) y la UI gana un segundo botón: correr con Claude Code o con el bucle. Se borran seis libros de prueba. | Apartar no es borrar: el valor del proyecto es poder correr **el mismo flujo con las mismas puertas** de forma determinista y de forma agéntica, y comparar. Lo que sí sobraba eran nueve libros de ejemplo de los que dos se usan. |
| **8.1** | 2026-09-16 | _sin commitear_ | `legado/demo.py` fuerza la mtime de las críticas del fixture por delante de su prosa al resetear. | La regla «una crítica más vieja que la prosa no abre G2» es correcta, pero en un `git clone` las mtimes se asignan en el orden que toque: el demo fallaba en una máquina recién clonada y no en la del que lo escribió. Un fixture tiene que ser reproducible en cualquier máquina o no es un fixture. |
| **8.0** | 2026-09-16 | _sin commitear_ | **El conductor por defecto pasa a ser Claude Code**, siguiendo la skill `dirigir-novela`: decide qué escena toca, despacha los subagentes, corre las puertas y reacciona a lo que digan, de la idea a `novela.md` y sin intervención humana. `crear_novela.py` queda como camino desatendido, no como el camino principal. Nuevos: `harness/scripts/crear_libro.py` (crear el canon sin preguntas, llamando al mismo `server.crear()` que usa la UI) y los agentes `escritor`, `corrector` y `lector-capitulo`. | Un bucle en Python es reproducible pero es tonto: ante un fallo repetido solo sabe reintentar el mismo camino tres veces y parar. El conductor agéntico puede replanificar la escena, leer el error y cambiar de estrategia. **Lo que NO se movió es quién acepta**: las puertas siguen siendo scripts y el conductor acata su JSON. Se cambia quién elige el camino, no quién pone el listón — si se movieran las dos cosas a la vez, el harness dejaría de validar nada. |
| **8.0** | 2026-09-16 | _sin commitear_ | **G3 deja de ser la puerta humana**: la lee el agente `lector-capitulo` y la decide `gate_chapter.py` (escenas aprobadas, presupuesto del capítulo, cita que exista de verdad, ningún hallazgo bloqueante). | G3 era el único punto donde el ciclo se paraba a esperar a una persona, y eso hacía imposible correr un libro entero de punta a punta. La regla que importaba no era «lo lee un humano» sino «nadie abre su propia puerta»: esa se mantiene intacta — el lector emite hallazgos con cita y el umbral lo aplica un script, igual que en G2. Lo que se pierde es el juicio humano sobre el capítulo; queda anotado como el precio de la autonomía, no como una mejora. |
| **8.0** | 2026-09-16 | _sin commitear_ | Al agotarse `intentos_max`, el conductor **replanifica la escena una vez** (beats nuevos, canon intacto) antes de parar; y si G4 no abre por un hilo sin cerrar, replanifica en vez de preguntar. | El camino en Python paraba y decía «decide vos». Sin persona delante eso es un cuelgue. Replanificar toca el plan, que es lo que el `planner` genera — nunca el canon, que sigue siendo lo único que solo una persona cambia (regla 4). El segundo agotamiento sí para: insistir más es quemar tokens contra el mismo muro. |
| **7.9** | 2026-09-16 | _sin commitear_ | El modelo de cada llamada sale de las claves de `modelUsage` (el que más salida generó), no del campo `model` — que `claude -p` no devuelve. Vale para la traza local y para Langfuse. | `traza.jsonl` venía guardando `modelo: null` en todas las llamadas desde que existe, y nadie lo notó porque la UI no lo mostraba. Es el dato del que cuelga el precio y con el que se compara una versión del prompt contra otra: sin él, «cuánto costó» no se puede atribuir a nada. |
| **7.9** | 2026-09-16 | _sin commitear_ | **Espejo del ciclo en Langfuse** (`harness/scripts/observabilidad.py`): una sesión por corrida, una traza por escena —con el intento, el agente que escribió, las dos lentes y cada puerta— y la rúbrica de G2 ingerida como *scores*. Cada `claude -p` va como `agent` con su `generation` dentro, que es donde viven modelo, tokens y costo reales. Es **opcional**: sin claves en `.env`, sin el paquete o sin red, `abrir()` devuelve un objeto nulo y el ciclo no cambia una línea. | `reports/traza.jsonl` responde «cómo funcionó **esta** corrida» y no puede responder «¿la rúbrica de este libro es peor que la del anterior?» ni «¿qué agente se lleva el presupuesto?»: son preguntas entre corridas y un JSONL por libro no las cruza. La traza local sigue siendo la dueña —es la que no necesita red y la que lee la UI—; esto es un espejo, y por eso no puede tumbar el ciclo si falla. La rúbrica va como *score* y no como metadato porque es la medida de calidad del harness: como score se filtra, se promedia y se compara entre versiones del prompt. |
