...
| RI-03 | `GET /novels/{id}` | `orchestration/` | — | Capítulo y escena en curso, capítulos congelados, cuarentenas, condición de cierre; el motivo, `reason`, sale del último `work.close` de la traza, también del que deja una tirada abortada (`srs-backend-v4.md` RF-238) |
...
| RF-20 | El punto de reanudación (PRO-14) se escribe en `run_state` al cerrar cada escena. Al arrancar con un capítulo sin congelar, se reanuda desde la última escena cerrada y se descarta todo borrador posterior. El punto conserva la cuenta de reintentos consumidos del capítulo y la escaleta vigente (`architecture.md` §7.4). Solo cubre escenas que pasaron su puerta y todas las anteriores a ellas (PRO-I2). Un capítulo ya congelado no se reescribe al reanudar | PRO-I2 | VER-06, VER-18 |
...
| RF-27 | Una escaleta que no pasa `outline.check` vuelve al Arquitecto con los defectos y su evidencia; pasa, se congela y ya no cambia salvo replanificación. La escaleta tiene los intentos de una replanificación de arco de RF-18, `retries.ARC_REPLANS`: dos | `architecture.md` §7.1 | VER-05 |
...
| RD-07 | Tablas de memoria de trabajo `wm_run_state`, `wm_draft`, `wm_defect`, `wm_verdict`, `wm_admission`. `wm_verdict` se crea aunque la versión 1 no tenga Jurado, para que el fichero sea completo. `wm_run_state` guarda además `chapter_attempts` y `arc_replans`, enteros no negativos con 0 por defecto, y `outline`, JSON o nulo, que entran en la migración 6 solo añadiendo y sin relleno: una tirada a medias de un fichero anterior reanuda con la cuenta a cero, y leer no migra | PRO-13 | VER-05 |
