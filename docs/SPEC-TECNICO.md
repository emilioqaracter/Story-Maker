...
| **No hace** | no juzga su capítulo; no cambia el plan; al corregir, no toca lo que no le señalaron; no anuncia que se revisó |
...
Antes de escribir el archivo del borrador relee lo que escribió y corrige él
mismo lo mecánico: ortografía y acentuación, palabras cortadas o repetidas,
concordancia, la grafía y el género de cada nombre y término del oficio, la
puntuación del diálogo, y la persona y el tiempo verbal del plan dentro de los
recuerdos y en la última frase. Está en su archivo, sección *Antes de
entregar*, y no en un juez: verlo no pide criterio, y una luz roja gastada en
una tilde le cuesta uno de sus tres intentos.
...
| **1.6** | 2026-09-18 | Sección *Antes de entregar* en `redactor.md`: la lista de lo mecánico que corrige sobre su propio borrador (§4). Sección *La leccion* reescrita en `revisor.md` y `verificador.md`, y dos reglas nuevas en la skill `luz`: la lección se escribe para acabar copiada en las instrucciones del redactor, con las palabras más comunes que el juez encuentre, y nunca sobre erratas (§4, §5). | Las novelas de control `control-01` y `control-02` dieron la evidencia: de trece lecciones, las que agrupaban eran las que nombraban un hábito de escritura, y las mecánicas —una tilde, un término que cambia de género— no agrupaban con nada y aun así consumían intentos. Que cada juez redactara la misma idea con su propio giro impedía contarlas: sin frases parecidas no hay lección repetida, y sin lección repetida el loop no tiene regla que escribir. |
| **1.7** | 2026-09-18 | `**Refuerzo:**` como última línea de toda luz verde, en `revisor.md`, `verificador.md` y la skill `luz` (formato, ejemplo y reglas). Sección *La leccion* de los dos jueces reescrita con la escalera caso / situación / capacidad, su prueba y seis ejemplos nuevos por juez (§4, §5). | Marcador distinto del de la lección porque el paso 5 de la skill `automejora` cuenta con `grep '^\*\*Leccion:\*\*'`: si el refuerzo compartiera etiqueta, entraría en la cuenta de la que sale la regla nueva y la contaminaría en cuanto los capítulos empezaran a salir bien. |
