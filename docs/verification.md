...
| Que en el perfil de extensión `prueba` el Jurado apruebe lo que en `novela` suspendería | Con `prueba` aprueba con mediana 2 y no 3 y ancla citas de 5 palabras y no de 8, porque una tirada de prueba mide el ciclo y no la calidad literaria; con `novela` nada cambia (`specs/srs-backend-v4.md` D-128) | Los niveles y el umbral de cada registro `jury` en la traza de VER-09, y la tabla por brief de VER-10 |
| Que en el perfil de extensión `prueba` el conjunto dorado de CAL-10 detecte peor | Con 3 escenas hay pocos fragmentos que sembrar, y con menos casos la tasa tiene más varianza; corre una vez, al congelar el último capítulo (`specs/srs-backend-v4.md` D-112) | La tasa del conjunto dorado en la traza de VER-09, que en el perfil `novela` se mide cada 5 capítulos |
