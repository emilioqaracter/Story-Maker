...
| **Regla de coincidencia** | La cita se normaliza —espacios, comillas tipográficas, guiones de diálogo, mayúsculas, y la barra de salto de párrafo y los puntos suspensivos tipográficos (D-65)— y debe aparecer **exactamente una vez** en la escena donde ancla, con **8 palabras o más**; las del Jurado en el perfil de extensión `prueba` (PRO-15), con 5 o más (`specs/srs-backend-v4.md` D-128). Nada de lematización ni de coincidencia difusa |
...
| Que se borren las últimas líneas de una traza, o el fichero entero | La cadena de hashes de VER-06 detecta una línea editada, borrada, insertada o reordenada, pero ningún eslabón apunta a las últimas; detectarlo exigiría un ancla fuera del fichero, y la traza no se duplica en el SQLite (`specs/srs-backend-v1.md` RI-17) | La traza se copia con la tirada, y los capítulos congelados del SQLite no cuadrarían con los `chapter.frozen` de la traza |
| Que un comando `Bash` ofuscado, con variables o `eval`, escape al hook de política | Sobre `Bash` las reglas se evalúan con expresiones regulares sobre el texto del comando; `Read` y `Write` se evalúan por ruta y no tienen este hueco | Cada decisión, también las que permiten, queda en `.claude/audit/policy.jsonl` con su cadena |
| Que el audit log de desarrollo se borre entero | `.claude/audit/policy.jsonl` es de cada máquina y ninguna de las cuatro reglas del hook lo protege (`specs/srs-backend-v4.md` D-92) | Su cadena señala cualquier borrado parcial; los hooks son del desarrollo, no de la novela |
| Que en los perfiles de extensión `prueba`, `breve` y `corta` el Jurado apruebe lo que en `novela` suspendería | Con `prueba` aprueba con mediana 2 y no 3 y ancla citas de 5 palabras y no de 8, y con `breve` y `corta` con mediana 2 y citas de 8, porque esas tiradas miden el ciclo y no la calidad literaria; con `novela` nada cambia (`specs/srs-backend-v4.md` D-128, D-132, D-139) | Los niveles y el umbral de cada registro `jury` en la traza de VER-09, y la tabla por brief de VER-10 |
| Que en los perfiles de extensión `breve`, `corta` y `prueba` se congelen capítulos con defectos S1 de modelo, o con S1 deterministas tras agotar la reparación | Con `lenient` esas tiradas prefieren una obra con defectos declarados a ninguna obra: la escena agotada se acepta, en la puerta de capítulo solo bloquean los S1 `check.*`, el Jurado no bloquea y la reparación agotada congela el mejor intento; con `novela` nada cambia (`specs/srs-backend-v4.md` D-136). Además, el modelo TLA+ del flujo de VER-18, en `backend/orchestration/model/`, describe el flujo estricto y no el permisivo | Los registros `scene.forced`, `chapter.forced` y `replan.kept` en la traza de VER-09, `chapter.gate` con lo que no bloqueó y el motivo de `work.close` con lo pendiente (RF-284) |
| Que la prosa narre como presente a un personaje que el Planificador puso en el elenco y no existe en el canon | El elenco se filtra a las entidades del canon y la congelación deja fuera de los presentes lo que quede, sin rechazar la salida: rechazarla gastaría los reintentos y, agotados, la tirada; vale en todos los perfiles (`specs/srs-backend-v4.md` D-140) | El `process.defect` de `agent` `planificador` en la traza de VER-09, con las entidades que se quedaron fuera |
| Que en el perfil de extensión `prueba` el conjunto dorado de CAL-10 detecte peor | Con 3 escenas hay pocos fragmentos que sembrar, y con un solo caso la tasa es 0 o 1; corre una vez, al congelar el último capítulo (`specs/srs-backend-v4.md` D-112, D-131) | La tasa del conjunto dorado en la traza de VER-09, que en el perfil `novela` se mide cada 5 capítulos |
| Que el bloque del encargo del Jurado pase de su cupo en ejecución | Ningún guardarraíl lo trunca ni lo rechaza; lo cubren las pruebas de VER-12 sobre el caso en el techo y una propiedad de VER-06, y el peor encargo de los cinco briefs mide 137 de 500 | La admisión rechaza toda llamada sobre 100.000, y el recuento real frente al estimado queda en la traza |
