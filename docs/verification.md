...
| **Regla de coincidencia** | La cita se normaliza —espacios, comillas tipográficas, guiones de diálogo, mayúsculas, y la barra de salto de párrafo y los puntos suspensivos tipográficos (D-65)— y debe aparecer **exactamente una vez** en la escena donde ancla, con **8 palabras o más**, también las del Jurado en todos los perfiles de extensión (PRO-15, `specs/srs-backend-v4.md` D-142). Nada de lematización ni de coincidencia difusa |
...
| Que en el perfil de extensión `prueba` el conjunto dorado de CAL-10 detecte peor | Con 3 escenas los 5 casos salen de pocos fragmentos, y corre una sola vez, al congelar el último capítulo (`specs/srs-backend-v4.md` D-112, D-142) | La tasa del conjunto dorado en la traza de VER-09, que en el perfil `novela` se mide cada 5 capítulos |
